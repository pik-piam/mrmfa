...
calcPlHVCinput <- function(subtype) {
  df <- readSource("LeviCullen", subtype = "HVCbyProcess", convert = FALSE) %>%
    as.data.frame() %>%
    rename("output" = "Data1", "input" = "Data2", "type" = "Data3", "coef" = "Value") %>%
    mutate(output = as.character(.data$output), input = as.character(.data$input), type = as.character(.data$type)) %>%
  ...
  # ---------------------------------------------------------------------------
  polymerization <- df %>%
    filter(.data$type == "plastics") %>%
    group_by(.data$output) %>%
    summarize(sum_inputs = sum(.data$coef)) %>%
    rename("polymer" = "output")
  ...
  get_HVC_inputs <- function(product, data, multiplier = 1) {
    rows <- data %>% filter(.data$output == product)
  ...
  HVC_input <- purrr::map_dfr(polymerization$polymer, function(p) {
    get_HVC_inputs(p, df) %>%
      group_by(.data$input) %>%
      summarise(tonnes = sum(.data$tonnes), .groups = "drop") %>%
      mutate(polymer = p) %>%
      select("polymer", "input", "tonnes")
  })

  # get carbon content of HVC
  carbon_content <- calcOutput("PlCarbonContent", subtype = "HVC", aggregate = FALSE) %>%
    as.data.frame() %>%
    select(-"Cell", -"Region", -"Year") %>%
    rename("element" = "Data2")
