...
#' \link[=calcPopulation]{calcPopulation} from
...
calcCoPopulation1900To2150 <- function(scenario = "SSP2") {
  ...
  future <- time_interpolate(popData$current, 2030:2150)
  popData$current <- mbind(popData$current[, 1960:2029], future)

  # extrapolate with Gapminder dataset as reference data for countries where such data exists
  pop <- toolBackcastByReference2D(x = popData$current, ref = popData$hist)

  # extrapolate with world average as reference data for other countries
  pop <- toolBackcastByReference2D(x = pop, ref = popData$worldHist)
...
getPopulation1900To2150Data <- function(scenario) {
  ...
  hist <- readSource("Gapminder", subtype = "population")
  hist <- hist[, 1900:2000]
