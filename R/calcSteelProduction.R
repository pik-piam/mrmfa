...
calcSteelProduction <- function() {
  prodData <- getSteelProductionData()
...
extrapolateSteelProduction <- function(prodData) {
  ...
  prod <- toolBackcastByReference2D(prodData$current, ref=prodData$recent, 
                                    doInterpolate=FALSE)  # already interpolated
  ...
  prod <- toolBackcastByReference2D(prod, ref=worldRef,
                                    doInterpolate=FALSE)  # already interpolated
